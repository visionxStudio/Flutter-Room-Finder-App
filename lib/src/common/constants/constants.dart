import 'package:flutter/material.dart';

/// tablet view breakpoint i.e. 768
const double kTabletBreakpoint = 768.0;

/// 18
const double kDefaultFontSize = 18;

///20
const double kDefaultPadding = 20;

///20
const double kDefaultmargin = 20;

///20
const double kDefaultRadius = 20;

const double kNavBarHeight = 75;

const Color kScafoldColor = Color(0xFFF5F6FA);
const Color kBottonColor = Color(0xFF4318FF);
const Color kTextLinkColor = Color(0xFF3311DB);
const Color kNavBarBackgroundColor = Color(0xFFF0EFFC);
const Color knavbarInactiveColor = Color(0xFF8F9BBA);
const Color kLightBlueTextColor = Color(0xff707EAE);
const Color kBlueVioletColor = Color(0xFF6236FF);

// Colors for chips
const Color kChipColor = Color(0xFFE9EDF7);
const Color kCoursesTextColor = Color(0xFF47548C);

const Color kPrimaryColor = Color(0xFF1B2559);
const Color kSecondaryColor = Color(0xFFE0F3FF);
const Color kPrimaryColorDark = Color(0xFF0B2B40);
const Color kWhiteColor = Color(0xFFFFFFFF);
const Color kBlackColor = Color(0xFF2B2B2B);
const Color kGreyColor = Color(0xFF81868A);
const Color kLightGreyColor = Color(0xFFD2D6D9);
const Color kLightestGreyColor = Color(0xFFEAEAEA);
const Color kOrangeColor = Color(0xFFF27457);
const Color kGreen = Color(0xFF27AE60);
const Color kPureBlack = Color(0xFF000000);

const Color kLightGrey = Color(0xFF12121D);
const Color kHomeBackground = Color(0xFFF6F8FD);

const Color kDarkGrey = Color(0xFFB0BBD5);
const Color kBlackGrey = Color(0xFF1A1A1A);

const Color kAppBarColor = Color(0XFF1B2559);
const Color kIndicatorColor1 = Color(0xFF9CECFB);
const Color kIndicatorColor2 = Color(0xFF65C7F7);
const Color kIndicatorColor3 = Color(0xFF0052D4);
const Color kSecondCurveColor = Color(0xFFBECBE3);

const Color kChatButtonColor = Color(0xFFBDBDBD);
const Color kSearch = Color(0xFFF4F7FE);
const Color ksearchText = Color(0xFFA3AED0);

const Color kSubtitleColor = Color(0xFF909093);

const Color kTopMenuUnselectedColor = Color(0xFFC9D4EA);

const Color kOutlineBorderColor = Color(0xFF304A87);

const Color kInputOptionButtonColor = Color(0xFF5E5E5E);

const Color kNavBackgroundColor = Color(0xff2B3674);

const Color kBottomSheetTitleColor = Color(0xFF272728);
const Color kBottomSheetSubTitleColor = Color(0xFF6E6E70);
const Color kBottomSheetArrowColor = Color(0xFF3E3E40);

const Color kOptionColor = Color(0xFF868687);

const Color kMessageTimeColor = Color(0xFFB4BBC6);

const Color kReplyMessageBorderColor = Color(0xFFE7E7ED);
const Color kUncheckedRadioColor = Color(0xFFa3aed0);
const Color kLightgreyColor = Color(0xFFa3aed0);

const Color kCalendarButtonColor = Color(0xFFE4E7EB);

const Color kStarbackgroundColor = Color(0xFFFFF7DF);
const Color kLikebackgroundColor = Color(0xFFD8FFE8);
const Color kLikeButtonColor = Color(0xFF6FCF97);
const Color krelationColor1 = Color(0xFF9CECFB);
const Color krelationColor2 = Color(0xFF65C7F7);
const Color krelationColor3 = Color(0xFF0052D4);

const Color ktextColor = Color(0xFF575758);
const Color kDividerColor = Color(0xFFE7E7E8);

const Color kmasterCardColor = Color(0xFFFFF1DC);

const Color kIconGradientColor1 = Color(0xFF10A9F3);
const Color kIconGradientColor2 = Color(0xFF2F00FF);
const Color kBlackText = Color(0xFF363B46);
